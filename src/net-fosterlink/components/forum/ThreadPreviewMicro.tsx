...
export const ThreadPreviewMicro = ({ thread } : {thread: ThreadModel}) => {
  ...
  return (
    <Card 
      className="p-3 mb-2 cursor-pointer hover:bg-accent/50 transition-colors drop-shadow-lg"
      onClick={() => navigate(`/threads/thread/${thread.id}`)}
    >
      <h4 className="font-semibold text-sm mb-1">{thread.title}</h4>
      <div className="text-xs text-muted-foreground">Posted at {new Date(thread.createdAt).toLocaleDateString()}</div>
